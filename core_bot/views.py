...
def generar_calendario_texto():
    ...
    texto += "`Lu Ma Mi Ju Vi Sa Do`\n" # Usamos formato código para que se alinee en Telegram
...
def enviar(chat_id, texto):
    requests.post(f"{TELEGRAM_API_URL}/sendMessage", data={'chat_id': chat_id, 'text': texto, 'parse_mode': 'Markdown'})


# ===== MÁQUINA DE ESTADOS (BOT) =====
def procesar_crm(chat_id, texto):
    chat_id = str(chat_id)
    alumno, created = Alumno.objects.get_or_create(telegram_id=chat_id)
    ...
    if alumno.estado_chat == 'ESPERANDO_NOMBRE':
        if validar_nombre(texto):
            ...
            enviar(chat_id, "❌ Debes ingresar 1 nombre y 1 apellido válidos.")

    elif alumno.estado_chat == 'ESPERANDO_TELEFONO':
        if validar_telefono(texto):
            # Guardamos el teléfono temporalmente en cita_fecha para no tener que modificar tu models.py
        ...
        try:
            ...
            if dia_semana == 6:
                ...
                # Agregamos la fecha al buffer temporal
        ...
        try:
            ...
            if hora:
                ...
                # Guardamos ya el registro final
